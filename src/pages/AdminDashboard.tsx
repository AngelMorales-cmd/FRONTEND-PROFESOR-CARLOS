...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = () => {
    try {
      ...
      const totalVoters = mockDB.getTotalVoters();
      const candidates = mockDB.getCandidates();

      const totalVotes = votes.length;
      const participationRate = totalVoters > 0 ? (totalVotes / totalVoters) * 100 : 0;

      const presidentialVotes = votes.filter((v) => v.category === "presidencial").length;
      const distritalVotes = votes.filter((v) => v.category === "distrital").length;
      const regionalVotes = votes.filter((v) => v.category === "regional").length;
      ...
      setCandidatesData([...candidates].sort((a, b) => b.vote_count - a.vote_count));
    } catch (error) {
  ...
  // Funciones de limpieza de datos (simuladas)
  ...
  const removeDuplicates = async () => {
    ...
    try {
      ...
      // Buscar votos duplicados por voter_dni y category
      votes.forEach((vote) => {
      ...
      if (duplicates.length > 0) {
        toast.success(`Se encontraron ${duplicates.length} votos duplicados (simulado)`, {
          description: "En un sistema real, estos serían eliminados",
        });
  ...
  const normalizeData = async () => {
    ...
    try {
      ...
      if (normalizedCount > 0) {
        toast.success(`Se normalizaron ${normalizedCount} registros (simulado)`);
        fetchDashboardData(); // Refrescar datos
      } else {
