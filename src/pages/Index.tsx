...
import { mockDB, type Candidate } from "@/data/mockData";
...
const Index = () => {
  ...
  const checkVotedCategories = (dni: string) => {
    try {
      const votes = mockDB.getVotesByVoter(dni);
  ...
  const fetchCandidates = () => {
    setLoading(true);
    try {
      // Obtener candidatos de los datos simulados
      const candidatesData = mockDB.getCandidates();
      setCandidates(candidatesData);
      const total = candidatesData.reduce((sum, c) => sum + c.vote_count, 0);
      setTotalVotes(total);
    } catch (error) {
  ...
  const handleVoteComplete = () => {
    ...
    setTimeout(() => {
      const votes = mockDB.getVotesByVoter(voterDni);
      const categories = votes.map((v) => v.category);
  ...
  const loadCompletedVotes = () => {
    if (!voterDni) return;
    
    setLoadingVotes(true);
    try {
      // Obtener los votos del votante
      const votesData = mockDB.getVotesByVoter(voterDni);

      if (votesData && votesData.length > 0) {
        // Obtener información de los candidatos
        const votes = votesData.map((vote) => {
          const candidate = mockDB.getCandidateById(vote.candidate_id);
          return {
